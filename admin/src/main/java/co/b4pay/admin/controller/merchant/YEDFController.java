...
import co.b4pay.admin.dao.YEDFDao;
...
@Controller
@RequestMapping("/yedf")
public class YEDFController extends BaseController {
    /**
     * 日志对象
     */
    private static final Logger logger = LoggerFactory.getLogger(YEDFController.class);

    @Autowired
    private YEDFService yedfService;
    @Autowired
    ...
    private Page<YEDFPayroll> pac = new Page<>();

    //    @Autowired
    private HmacSHA1Signature signature = new HmacSHA1Signature();

    @RequestMapping(value = "list", method = RequestMethod.GET)
    @RequiresPermissions("yedf:list")
    public String list(Model model, @PageAttribute Page<YEDFPayroll> page, HttpServletRequest request) {
        ...
        if (roleIds.contains("1") || roleIds.contains("2")) {   //如果拥有超级管理员权限
            ...
            model.addAttribute("page", yedfService.findPage(page));
            Double successRate = yedfService.getSuccessRate(page);
            pac = page;
            if (paramsc != null) {
                try {
            ...
            String cIds = yedfService.findAdminChannel(aid);
            params.put("channelIds", cIds);
            page.setParams(params);
            Page<YEDFPayroll> pc = yedfService.findPage(page);
            Double successRate = yedfService.getSuccessRate(page);
        ...
        Double aDouble = yedfService.sumMoney(page);
        Double accountMoney = yedfService.sumAccountMoney(page) == null ? 0D :
                yedfService.sumAccountMoney(page);
        Integer accountCount = yedfService.accountCount(page) == null ? 0 :
                yedfService.accountCount(page);
        ...
        if (aDouble != null) {
            ...
            Double d = yedfService.sumMoney(page);
            if (d == null) {
                d = 0D;
            }
            Double dd = yedfService.sumAccountMoney(page);
            if (dd == null) {
                dd = 0D;
            }
        ...
        if (page.getParams() != null &&
            ...
            Merchant merchant = merchantService.get(page.getParams().getString("merchantId"));
            BigDecimal accountFrozen = merchant.getAccountFrozen();
            model.addAttribute("accountFrozen", accountFrozen);
        }

        return "YEDF/YEDFList";
    }

    @RequestMapping(value = "form")
    @RequiresPermissions("yedf:form")
    public String form(Model model, String id) {
        model.addAttribute("consume", yedfService.get(id));
        return "YEDF/YEDFForm";
    }
    ...
    public String updateStatus(RedirectAttributes redirectAttributes, String id, int status, String amount) {
        ...
        try {
            ...
            yedfService.updateStatus(id, status, getBigDecimal(amount));

            YEDFPayroll trade = yedfService.get(id);
...
//        returnData.put("tradeId",id);
            ...
            yedfService.updateTrade(id, returnData.toString(), 0);

            JobTrade jobTrade = yedfService.findJobTradeById(trade.getId());

            //发起给客户回调任务
            JSONObject params = new JSONObject();
            params.put("mark", "admin发起的回调");
            //new Thread(new YEDFController().NotitfTask(jobTrade, trade)).start();
    ...
    public void derived(HttpServletRequest request,
        ...
        pac.setPageFirst(0);
        List<Object> consumeDTOS = toConsumeDTO(yedfService.findPage(pac).getList());
    ...
    public static List<Object> toConsumeDTO(List<YEDFPayroll> consumes) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        List<Object> consumeDTOS = new ArrayList<>();
        for (int i = 0; i < consumes.size(); i++) {
            ConsumeDTO consumeDTO = new ConsumeDTO();
            YEDFPayroll consume = consumes.get(i);
