    ...
    <div class="admin-content-body">
        ...
        <div class="am-g">
            <div class="am-u-sm-6" style="width: 30%;">
                <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                        ...
                        <br>
                        <%--<strong class="am-text-primary am-text-lg">交易支付金额：${not empty sumRequestMoney ? sumRequestMoney : "0.0"}元</strong>
                        /
                        <small>${page.totalCount}笔</small>
                        <br>--%>
            ...
            <div class="am-u-sm-6" style="width: 70%;">
                <form id="searchForm" action="${ctx}/consume/list" method="get" class="am-form-inline">
                    ...
                    <shiro:hasAnyRoles name="admin,superadmin">
                        <div class="am-input-group am-input-group-sm">
                            <select data-am-selected="{btnWidth: '120px'}" placeholder="筛选地址" name="addressId"
                                    id="addressId" onchange="$('#searchForm').submit()">
                                <option value=" ">所有通道</option>
                                <c:forEach items="#{addressList}" var="address">
                                    <option value="${address.id}"
                                            <c:if test="${address.id eq page.params.addressId}">selected</c:if>>${address.mallName}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </shiro:hasAnyRoles>
                    ...
                    <table id="contentTable" class="am-table am-table-striped am-table-hover table-main">
                        ...
                        <c:forEach items="${page.list}" var="consume" varStatus="status">
                            <tr>
                                ...
                                <td>
                                    <c:if test="${consume.fzStatus == 0}">还未分账</c:if>
                                    <c:if test="${consume.fzStatus == 1}">分账成功</c:if>
                                    <c:if test="${consume.fzStatus == 2}">分账失败</c:if>
                                </td>
                                <td>${consume.paymentTime == null?'':consume.paymentTime}</td>
                                <td>
                                    <a href="${ctx}/consume/form?id=${consume.id}">详情</a>
                                    <shiro:hasAnyRoles name="admin,superadmin">
