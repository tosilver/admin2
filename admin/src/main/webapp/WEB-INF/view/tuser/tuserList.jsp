    ...
    <div class="admin-content-body">
        <div class="am-cf am-padding am-padding-bottom-0">
            <div class="am-fl am-cf">
                <strong class="am-text-primary am-text-lg">收款个码管理</strong> /
                <small>收款个码信息</small>
            </div>
        </div>
        <hr>
        <div class="am-g">
                ...
                <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                        <button type="button" class="am-btn am-btn-default"
                                onclick="window.location='${ctx}/tuser/form'">
            ...
            <div class="am-u-sm-6" style="width: 70%;">
                <form id="searchForm" action="${ctx}/tuser/list" method="get" class="am-form-inline">
                    ...
                    <div class="am-input-group am-input-group-sm">
                        <select data-am-selected="{btnWidth: '100px'}" placeholder="状态" name="status"
                            ...
                            <%--<option value="0" <c:if test="${page.params.status eq 0}">selected</c:if>>已冻结</option>--%>
                        </select>
                    </div>
                    <%--<div class="am-input-group am-input-group-sm">
            ...
            <div class="am-u-sm-12">
                <form class="am-form">
                    <table id="contentTable" class="am-table am-table-striped am-table-hover table-main">
                        ...
                        <tbody>
                        <c:forEach items="${page.list}" var="tuser" varStatus="status">
                            <tr>
                                <td>${status.index + 1}</td>
                                <td>${tuser.account}</td>
                                <td>${tuser.userid}</td>
                                <td>${tuser.payType == 1?"支付宝":"微信"}</td>
                                <td>${tuser.status== 0?"<font color='#FF0000'>停用</font>":"可用"}</td>
                                <td>
                                    <a href="${ctx}/tuser/form?id=${tuser.account}">详情</a>
                                    <a href="${ctx}/tuser/updateStatus?id=${tuser.account}&status=0"
                                       onclick="return confirm('确认要停用该账号吗？', this.href)">停用</a>
                                    <a href="${ctx}/tuser/updateStatus?id=${tuser.account}&status=1"
