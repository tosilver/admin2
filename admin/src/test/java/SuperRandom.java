...
public class SuperRandom {
     ...
     * @return
     */
    private String getCity(String first) {
     ...
     * @return
     */
    private String getArea(String provinceId, String two) {
        List<City> cityList = cityService.getByCity(provinceId, two);
        String cityId = null;
    ...
    public Map getMapAddress() {
        ...
        List<Province> byProvince = provinceService.getByProvince(province);
        String provinceId = null;
        for (Province province1 : byProvince) {
            provinceId = province1.getProvinceId();
        }
        map.put("province", province);
        String city = getCity(province);
        map.put("city", city);
        String area = getArea(provinceId, city);
        map.put("area", area);
        return map;
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public JSONArray getJsonListAddress(int num) {
        JSONArray json = new JSONArray();
        for (int i = 0; i < num; i++) {
            getMapAddress();
            json.add(getMapAddress());
        }
        return json;
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public List<UTPersonInfo> getList(int num) {
        List<UTPersonInfo> list = new ArrayList<UTPersonInfo>();
        for (int i = 0; i < num; i++) {
            UTPersonInfo info = new UTPersonInfo();
    ...
    /**
     * [(第一个参数为excl文件路径,文件不存在直接创建),(第二个参数为list集合)]
     * [生成excel表格(导入下面的jar包)]
     *
     * @param path
     * @param list
...
/*   <dependency>
    ...
    public void createExcel(String path, List<UTPersonInfo> list) {
        try {
            ...
            Label labelProvince = new Label(2, 0, "省");
            Label labelCity = new Label(3, 0, "市");
            Label labelArea = new Label(4, 0, "区");
            Label labelPhone = new Label(5, 0, "电话");
            Label labelroad = new Label(6, 0, "地址");
            ws.addCell(labelName);
            ws.addCell(labelSex);
            ws.addCell(labelProvince);
            ws.addCell(labelCity);
            ws.addCell(labelArea);
            ws.addCell(labelPhone);
            ws.addCell(labelroad);

            for (int i = 0; i < list.size(); i++) {
                Label name = new Label(0, i + 1, list.get(i).getName());
                Label sex = new Label(1, i + 1, list.get(i).getSex());
                Label province = new Label(2, i + 1, list.get(i).getProvince());
                Label city = new Label(3, i + 1, list.get(i).getCity());
                Label area = new Label(4, i + 1, list.get(i).getArea());
                Label tel = new Label(5, i + 1, list.get(i).getTel());
                Label road = new Label(6, i + 1, list.get(i).getRoad());
                ws.addCell(name);
                ws.addCell(sex);
                ws.addCell(province);
                ws.addCell(city);
    ...
    @Test
    public void test() {
        //           System.out.println(getChineseName());//姓名
        //           System.out.println(name_sex);//性别
        //           System.out.println(getTel());//电话
        //           System.out.println(getRoad());//住宅
        //           System.out.println(getEmail(6,9));//邮箱
        //           System.out.println(getJsonAddress());//json格式单条
        //           System.out.println(getMapAddress());//map格式
        //          System.out.println(getJsonListAddress(3));//json格式多条
        long start = System.currentTimeMillis();
        //生成表格(无格式的)
        createExcel("D:信息表.xls", getList(426));
